...
public class ClientAuthenticationFilter extends ClientsConfigFilter {
    ...
    protected void saveUserProfile(CommonProfile profile, HttpServletRequest request) {
        if (isStateless()) {
            ...
            UserUtils.setProfile(request.getSession(true), profile);
        }
    }
    ...
    protected boolean hasAccess(CommonProfile profile, HttpServletRequest request) {
        //TODO we should extract this code from play pac4j in pac4j-core (but also incude commons-lang)
        boolean access = true;
    ...
    protected void saveOriginalUrl(HttpServletRequest request) {
        ...
        request.getSession(true).setAttribute(ORIGINAL_REQUESTED_URL, requestedUrl);
    }

    protected String retrieveOriginalUrl(HttpServletRequest request) {
        return (String) request.getSession(true).getAttribute(ORIGINAL_REQUESTED_URL);
    }
